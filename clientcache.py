    ...
    try:
        ...
        if csv_file and csv_file.filename.endswith('.csv'):
            ...
            try:
                ...
                df['ACC'] = df['ACC'].astype(np.int64)
                df['CALLNO'] = df['CALLNO'].astype(np.int64)
                df['ISBN'] = df['PRICE'].astype(np.int64)
    ...
    try:
        ...
        if csv_file and csv_file.filename.endswith('.csv'):
            ...
            try:
                ...
                df['CATALOGNO'] = df['CATALOGNO'].astype(np.int64)
                df['YEAR'] = df['YEAR'].astype(np.int64)
            ...
            return render_template("inventory_cds_add.html", success_message="Book file imported successfully")
        else:
            flash('Invalid file type. Please upload a CSV file.')
            return redirect('/')
    except Exception as e:
...
def import_thesis_csv():
    try:
        ...
        if csv_file and csv_file.filename.endswith('.csv'):
            ...
            return render_template("inventory_thesis_add.html", success_message="Book file imported successfully")
        else:
            flash('Invalid file type. Please upload a CSV file.')
            return redirect('/')
    except Exception as e:
        return render_template("inventory_thesis_add.html", success_message="An error occurred while trying to add data ")
    ...
    try:
        ...
        if csv_file and csv_file.filename.endswith('.csv'):
            ...
            try:
                ...
                df['ACC'] = df['ACC'].astype(np.int64)
                df['CALLNO'] = df['CALLNO'].astype(np.int64)
                df['ISSUENO'] = df['ISSUENO'].astype(np.int64)
                df['YEAR'] = df['YEAR'].astype(np.int64)
                df['ISBN'] = df['ISBN'].astype(np.int64)
            ...
            return render_template("inventory_journals_add.html", success_message="Book file imported successfully")
        else:
            flash('Invalid file type. Please upload a CSV file.')
            return redirect('/')
    except Exception as e:
        return render_template("inventory_journals_add.html", success_message="An error occurred while trying to add data ")
...
def import_projects_csv():
    try:
        ...
        if csv_file and csv_file.filename.endswith('.csv'):
            ...
            return render_template("inventory_projects_add.html", success_message="Book file imported successfully")
        else:
            flash('Invalid file type. Please upload a CSV file.')
            return redirect('/')
    except Exception as e:
        return render_template("inventory_projects_add.html", success_message="An error occurred while trying to add data ")
...
def import_eresource_csv():
    try:
        ...
        if csv_file and csv_file.filename.endswith('.csv'):
            ...
            return render_template("inventory_eresource_add.html", success_message="Book file imported successfully")
        else:
            flash('Invalid file type. Please upload a CSV file.')
            return redirect('/')
    except Exception as e:
        return render_template("inventory_eresource_add.html", success_message="An error occurred while trying to add data ")
        ...
        if reg_no != '':
            ...
            if phone_no != '':
                ...
                if name != '' and designation == '' and dept == '' and year == '':
                    data = list(collection.find({"NAME": {"$regex": name}}, {'_id': 0, 'SNO': 0, }))
                    ...
                    data = list(
                        collection.find({"NAME": {"$regex": name}, "DESIGNATION": {"$regex": designation}}, {'_id': 0, 'SNO': 0}))
                    

                elif name != '' and designation == '' and dept != '' and year == '':
                    data = list(collection.find({"NAME": {"$regex": name}, "DEPARTMENT": dept}, {'_id': 0, 'SNO': 0}))
                    

                elif name != '' and designation == '' and dept == '' and year != '':
                    data = list(collection.find({"NAME": {"$regex": name}, "YEAR": int(year)}, {'_id': 0, 'SNO': 0}))
                    ...
                    data = list(collection.find({"NAME": {"$regex": name}, "DESIGNATION": {"$regex": designation}, "DEPARTMENT": dept},
                                                {'_id': 0, 'SNO': 0}))
                    

                elif name != '' and designation != '' and dept == '' and year != '':
                    data = list(
                        collection.find({"NAME": {"$regex": name}, "DESIGNATION": {"$regex": designation}, "YEAR": int(year)},
                                        {'_id': 0, 'SNO': 0}))
                    

                elif name != '' and designation == '' and dept != '' and year != '':
                    data = list(
                        collection.find({"NAME": {"$regex": name}, "DEPARTMENT": dept, "YEAR": int(year)},
                                        {'_id': 0, 'SNO': 0}))
                    ...
                    data = list(collection.find(
                        {"NAME": {"$regex": name}, "DESIGNATION": {"$regex": designation}, "DEPARTMENT": dept, "YEAR": int(year)},
                        {'_id': 0, 'SNO': 0}))
...
def book_input():
    try:
        ...
        if acc_no != '':
            data = list(collection.find({"ACC": int(acc_no)}, {'_id': 0, 'SNO': 0, }))
            if data == []:
...
def book_input_sub():
    try:
        ...
        if acc_no != '':
            ...
            if call_no != '':
                data = list(
                    collection.find({"CALLNO": {"$regex": int(call_no), "$options": "i"}}, {'_id': 0, 'SNO': 0, }))
...
def submit_data(user_id, acc_no):
    try:
        ...
        try:
            ...
            bookData = list(book_data_collection.find({"ACC": int(acc_no)}))
        ...
        if data and bookData:
            ...
            return render_template('bookmanager.html', user_id=user_id, name=name, desig=desig, department=department,
                                   ...
                                   due_details=due_details, fineamount=total_fine)
        elif not data:
...
def issue_book(user_id, acc_no):
    # if acc_no == None:
    #     return render_template('bookmanager.html', success_message=" ACC No must be entred")
    try:
        ...
        if data and bookData:
            ...
            if issued_book_count + due_book_count < 3:
                ...
                book_data_collection.update_one({'ACC': int(acc_no)}, {'$set': {'Status': 'Currently Issued'}})
                ...
                return render_template('bookmanager.html', error="User already has three books",
        ...
        try:
            ...
            if issue_data:
                ...
                if due_data:
                    ...
                    due_date = datetime.strptime(issue_data['ISSUEDATE'], '%d-%m-%Y')

                # Calculate the fine amount
                returned_date = datetime.now()
                fine_amount = (returned_date - due_date).days * 2 if due_data else 0
...
def return_book(user_id, acc_no):
    try:
        ...
        try:
            ...
            # Check if the book is in the IssueLog collection
            issue_data = issue_log_collection.find_one({'REGISTERNO': user_id, 'ACC': int(acc_no)})

            if issue_data:
                
                # Process for books in the IssueLog collection
                due_data = due_log_collection.find_one({'REGISTERNO': user_id, 'ACC': int(acc_no)})
                ...
                # Delete the issue log
                issue_log_collection.delete_one({'REGISTERNO': user_id, 'ACC': int(acc_no)})
                ...
                # Delete the due log
                due_log_collection.delete_one({'REGISTERNO': user_id, 'ACC': int(acc_no)})
               

            # Update the book status in the MongoDB
            book_data_collection.update_one({'ACC': int(acc_no)}, {'$set': {'Status': 'Available'}})
...
def renew_book(user_id, acc_no):
    try:
        ...
        query = {'REGISTERNO': user_id, 'ACC': int(acc_no)}
...
def quick_return_route_sub():
    try:
        ...
        if not acc_no:
            return render_template('bookmanagersub.html', error="Missing acc_no parameter.",
                                   success_message="Missing acc_no parameter.")

        try:
            acc_no = int(acc_no)
        except ValueError:
            return render_template('bookmanagersub.html', error="Invalid acc_no parameter.",
                                   success_message="Invalid acc_no parameter.")
        ...
        return render_template("bookmanagersub.html", success_message="An error occurred while trying to search data ")
...
def search_remove():
    try:
        ...
        collection = db['BookData']
        access_number = int(request.form.get('acc_no'))
        btn = request.form.get('submit')
        if btn == 'submit':
            found = collection.find_one({'ACCNO': access_number}, {'_id': 0})
            if found:
                print("")
...
def remove():
    try:
        ...
        collection.delete_one({'ACCNO': int(access_number)})
    except Exception as e:
...
def log_thesis():
    try:
        ...
        page_no = request.form.get('page_no').strip()
        date = request.form.get('date_of_defense').strip()
        isbn = request.form.get('isbn').strip()
        link = request.form.get('repository_link').strip()
...
def log_journal():
    try:
        ...
        call_no = request.form.get('call_no').strip()
        title = request.form.get('title').strip()
        subject = request.form.get('subject_area').strip()
        publisher = request.form.get('publisher').strip()
        year = request.form.get('year').strip()
        page_no = request.form.get('page_no').strip()
        issue_no = request.form.get('issue_no').strip()
        isbn = request.form.get('isbn').strip()
        link = request.form.get('acc_link').strip()
...
def log_cds():
    try:
        acc_no = request.form.get('acc_no').strip()
        forma = request.form.get('format').strip()
        title = request.form.get('title').strip()
        author = request.form.get('author').strip()
...
def log_project():
    try:
        ...
        advisor = request.form.get('advisor').strip()
        title = request.form.get('title').strip()
        member= request.form.get('student').strip()
...
def search_missing():
    try:
        ...
        if btn == 'submit':
            collection = db['BookData']
            found = collection.find_one({'ACC': int(access_number)}, {'_id': 0})
            if found:
            ...
            mark_unavailable()
            return render_template('inventory_book_missing.html', success_message="Marked as Unavailable")
    except Exception as e:
        return render_template('inventory_book_missing.html')
...
def mark_unavailable():
    try:
        ...
        collection = db['BookData']
        found = collection.find_one({'ACC': int(access_number)}, {'_id': 0})
        collection.update_one({'ACC': int(access_number)}, {'$set': {'Status': 'unavailable'}})
